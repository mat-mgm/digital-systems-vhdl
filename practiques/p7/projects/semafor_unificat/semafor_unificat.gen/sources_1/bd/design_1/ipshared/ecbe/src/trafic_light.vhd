...
ilumination: process(div_clk1,green_c, ambar_c, red_c, green_v, red_v) begin -- green_c, ambar_c, red_c, green_v, red_v --(clk,green_c, ambar_c, red_c, green_v, red_v)
-- Cotxes
    if green_c = '1' then
        char3 <= "1100";
...
-- Temporitzador
timer: process begin
    if enable = '1' then -- Temporitzador comptant intermitent
        if enable_b = '1' then
            if div_clk1 = '1' then -- Intermitent encès
                bcdcount <= to_bcd(counter);
                char0 <= bcdcount(3 downto 0);
                char1 <= bcdcount(7 downto 4);
                wait for 500 ms;
            else -- Intermitent apagat
                char0 <= "1101";
                char1 <= "1101";
            end if;
        else -- Temporitzador comptant de forma contínua
            bcdcount <= to_bcd(counter);
            char0 <= bcdcount(3 downto 0);
            char1 <= bcdcount(7 downto 4);
        end if;
    else -- Temporitzador apagat
        char0 <= "1101";
        char1 <= "1101";
    end if;
end process;

-- Valors de cadascun dels displays que s'alternen segons el div_clk (digit_act)
selector: process(clk, digit_act, sg, char0, char1, char2, char3) begin
...
-- Codificació del semàfor
        "00111001" when "1010", -- Semï¿½for vermell
        "00000011" when "1011", -- Semï¿½for ï¿½mbar
        "11000101" when "1100", -- Semï¿½for verd
        "11111111" when "1101", -- Semï¿½for apagat
